...
public class SdkServer {
    ...
    public static void start() {
        try {
            ...
            https_config.setSecurePort(SysConfig.httpsPort);
            https_config.setOutputBufferSize(32768);
            https_config.addCustomizer(new SecureRequestCustomizer());

            SslContextFactory sslContextFactory = new SslContextFactory();
            sslContextFactory.setKeyStorePath("config/jetty.jks");
            sslContextFactory.setKeyStorePassword(SysConfig.httpsPwd);
            sslContextFactory.setKeyManagerPassword(SysConfig.httpsPwd);

            ServerConnector httpsConnector = new ServerConnector(server,
                    new SslConnectionFactory(sslContextFactory, "http/1.1"),
                    new HttpConnectionFactory(https_config));
            httpsConnector.setPort(SysConfig.httpsPort);
